		...
		<?php
			$post = get_post( $comment->comment_post_ID );
                        ...
                        if ( has_post_thumbnail( $comment->comment_post_ID ) ) 
                        {
                            ...
                            echo sprintf( __('<a href="%s" class="pg-woocommerce-imgsrc">%s</a>','profilegrid-woocommerce'), $product_link, $image );
                        } 
                        else 
                        {
                            echo sprintf( __('<img src="%s" alt="%s" class="pg-woocommerce-imgsrc" />','profilegrid-woocommerce'), wc_placeholder_img_src(), __( 'Placeholder', 'profilegrid-woocommerce' ) );
                        }
                ?>
			
		</div>
		
		<span class="pg-woocommerce-title"><a href="<?php echo $product_link; ?>"><span><?php echo get_the_title( $comment->comment_post_ID ); ?></span></a></span>
		<span class="pg-woocommerce-price"><?php echo $product->get_price_html(); ?></span>
		<span class="pg-woocommerce-avg" data-number="5" data-score="<?php echo $rating; ?>"></span>
		<span class="pg-woocommerce-product-review"><?php echo '&ldquo;' . $comment->comment_content . '&rdquo;'; ?></span>
